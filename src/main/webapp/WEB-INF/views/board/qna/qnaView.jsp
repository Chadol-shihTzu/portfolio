      ...
      <div class="col-md-12">
		...
		<main>
		    <div class="input-group mb-3">
		    	...
		    	<input type="text" class="form-control" id="qnaTitle" value="${view.qnaTitle}" name="qnaTitle" readonly>
		    </div>
		      <div class="input-group mb-3">
		      	<span class="input-group-text">작성자</span>
		      	<input type="text" class="form-control" id="qnaWriter"  name="qnaWriter" value="${view.qnaWriter}" readonly="readonly"/> 
		      	<span class="input-group-text">작성일</span>
		    	<input type="text" class="regDate form-control" value="<fmt:formatDate value="${view.qnaregDate}" pattern="yyyy-MM-dd hh:mm:ss"/>" readonly="readonly">
	      	  </div>
		      <textarea cols="" rows="20" class="content_textarea form-control" id="qnaContent"  name="qnaContent" readonly="readonly">${view.qnaContent}</textarea>
	    </main>
			...
			<c:if test="${member.userId == view.qnaWriter}">
      		<div class="d-flex justify-content-end">
      			<div class="btn-group mb-5">
      				<a href="${path}/board/qna/qnaUpdate?qnaNo=${view.qnaNo}" class="btn btn-outline-success">수정</a>
					<a href="${path}/board/qna/qnaDelete?qnaNo=${view.qnaNo}" class="btn btn-outline-danger">삭제</a>
      				<a class="btn btn-outline-primary" href="${path}/board/qna/qnalistPageSearch?num=1" >목록보기</a>
      			</div>
      		</div>
      		</c:if>
				...
				<c:forEach items="${reply}" var="reply">
					<div class="d-flex justify-content-between my-3">
						...
						<div class="btn-group" style="height:40px;">
							<a href="${path}/board/qna/replyModify?qnaNo=${view.qnaNo}&qnaRno=${reply.qnaRno}" class="text-nowrap border btn reply_modify_btn" style="height:36px;">수정</a><!-- btn-outline-secondary  -->
							<a href="${path}/board/qna/replyDelete?qnaNo=${view.qnaNo}&qnaRno=${reply.qnaRno}" class="text-nowrap border btn reply_delete_btn" style="height:36px;">삭제</a><!-- btn-outline-danger -->
						</div>
						</c:if>
...
<form method="post" action="${path}/board/qna/write">
	<div class="row g-2">
		<div class="col-auto">
	...
	댓글이 작성되는 현재페이지는 게시물을 순번으로 조회하고 있는 상태 루트/view?bno=? 여러개의 순번중에 인지할수 있도록 히든 필드를 써줌
	 -->
	<input type="hidden" name="qnaNo" value="${view.qnaNo}"/>
	<div class="btn-group">

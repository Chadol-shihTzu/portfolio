      ...
      <div class="col-md-12">
		...
		<main>
		    <div class="input-group mb-3">
		    	...
		    	<input type="text" class="form-control" id="poemTitle" value="${view.poemTitle}" name="poemTitle" readonly>
		    </div>
		    <div class="input-group mb-3">
		    	 <span class="input-group-text">인상 깊은 구절</span>
		    	<input type="text" class="form-control" id="poemShortTitle" value="${view.poemShortTitle}" name="poemShortTitle" readonly>
		    </div>
		      <div class="input-group mb-3">
		      	<span class="input-group-text">작성자</span>
		      	<input type="text" class="form-control" id="poemWriter"  name="poemWriter" value="${view.poemWriter}" readonly="readonly"/> 
		      	<span class="input-group-text">작성일</span>
		    	<input type="text" class="regDate form-control" value="<fmt:formatDate value="${view.poemregDate}" pattern="yyyy-MM-dd hh:mm:ss"/>" readonly="readonly">
	      	  </div>
		      <textarea cols="" rows="20" class="content_textarea form-control" id="poemContent"  name="poemContent" readonly="readonly">${view.poemContent}</textarea>
	    </main>
			...
			<c:if test="${member.userId == view.poemWriter}">
      		<div class="d-flex justify-content-end">
      			<div class="btn-group mb-5">
      				<a href="${path}/board/literature/poem/poemUpdate?poemNo=${view.poemNo}" class="btn btn-outline-success">수정</a>
					<a href="${path}/board/literature/poem/poemDelete?poemNo=${view.poemNo}" class="btn btn-outline-danger">삭제</a>
      				<a class="btn btn-outline-primary" href="${path}/board/literature/poem/poemlistPageSearch?num=1" >목록보기</a>
      			</div>
      		</div>
      		</c:if>
				...
				<c:forEach items="${reply}" var="reply">
					<div class="d-flex justify-content-between my-3">
						...
						<div class="btn-group" style="height:40px;">
							<a href="${path}/board/literature/poem/replyModify?poemNo=${view.poemNo}&poemRno=${reply.poemRno}" class="text-nowrap border btn reply_modify_btn" style="height:36px;">수정</a><!-- btn-outline-secondary  -->
							<a href="${path}/board/literature/poem/replyDelete?poemNo=${view.poemNo}&poemRno=${reply.poemRno}" class="text-nowrap border btn reply_delete_btn" style="height:36px;">삭제</a><!-- btn-outline-danger -->
						</div>
						</c:if>
...
<form method="post" action="${path}/board/literature/poem/write">
	<div class="row g-2">
		<div class="col-auto">
	...
	댓글이 작성되는 현재페이지는 게시물을 순번으로 조회하고 있는 상태 루트/view?bno=? 여러개의 순번중에 인지할수 있도록 히든 필드를 써줌
	 -->
	<input type="hidden" name="poemNo" value="${view.poemNo}"/>
	<div class="btn-group">

...
import com.portfolio.jdh.persistence.novelBoardDAO;

@Service
public class novelBoardServiceImpl implements novelBoardService{
	
	@Inject
	private novelBoardDAO dao;
	...
	public novelBoardVO view(int novelNo, HttpSession session) throws Exception {
	    ...
	    if (!viewedPostNumbers.contains(novelNo)) {
	        dao.boardHit(novelNo); // 조회수 증가
	        viewedPostNumbers.add(novelNo); // 세션에 추가
	    }

	    // 세션을 갱신합니다.
	    session.setAttribute("viewedPostNumbers", viewedPostNumbers);
		return dao.view(novelNo);
	}
	...
	@Override
	public List<novelBoardVO> listPageSearch(int displayPost, int postNum, String searchType, String keyword)throws Exception {

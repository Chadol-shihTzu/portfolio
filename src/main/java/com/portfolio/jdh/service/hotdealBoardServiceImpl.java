...
import com.portfolio.jdh.persistence.hotdealBoardDAO;

@Service
public class hotdealBoardServiceImpl implements hotdealBoardService{
	
	@Inject
	private hotdealBoardDAO dao;
	...
	public hotdealBoardVO view(int hotNo, HttpSession session) throws Exception {
	    ...
	    if (!viewedPostNumbers.contains(hotNo)) {
	        dao.boardHit(hotNo); // 조회수 증가
	        viewedPostNumbers.add(hotNo); // 세션에 추가
	    }

	    // 세션을 갱신합니다.
	    session.setAttribute("viewedPostNumbers", viewedPostNumbers);
		return dao.view(hotNo);
	}

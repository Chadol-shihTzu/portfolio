...
import com.portfolio.jdh.persistence.annoBoardDAO;

@Service
public class annoBoardServiceImpl implements annoBoardService{
	
	@Inject
	private annoBoardDAO dao;
	...
	@Override
	public annoBoardVO view(int annoNo, HttpSession session) throws Exception {
	    try {
	        ...
	        if (dao != null) {
	            ...
	            if (!viewedPostNumbers.contains(annoNo)) {
	                dao.boardHit(annoNo); // 조회수 증가
	                viewedPostNumbers.add(annoNo); // 세션에 추가
	            }
	            ...
	            annoBoardVO result = dao.view(annoNo);
